...
const SingleMenuItem = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const userRatingResponse = await axios.get(
          `http://localhost:5000/ratings/${id}/${userID}`
        );
        if (userRatingResponse.data.rating) {
          setUserRating(userRatingResponse.data.rating.rating);
        }
        const ratingCountResponse = await axios.get(
          `http://localhost:5000/count/${id}`
        );
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(
        <FaStar key={filledStars + i + (hasHalfStar ? 1 : 0)} color="grey" />
      );
    }
    return stars;
  };
